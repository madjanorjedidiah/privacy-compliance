...
class OrgProfile(TimeStampedModel):
    ...
    sectors = models.JSONField(default=list, help_text=f'Choices: {[c[0] for c in Sector.choices]}')
    data_categories = models.JSONField(default=list, help_text=f'Choices: {[c[0] for c in DataCategory.choices]}')
    processing_purposes = models.JSONField(default=list, help_text=f'Choices: {[c[0] for c in ProcessingPurpose.choices]}')
